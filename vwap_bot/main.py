...
def main():
    ...
    try:
        ...
        # فحص فوري لآلية الحفظ — لا ننتظر أول إشارة لنكتشف عطلاً.
        # يهمّ خصوصاً في عطلة نهاية الأسبوع: حارس السبت/الأحد يقفز فوق الرفع
        # الدوري، فلولا هذا الفحص لبقيت المزامنة مجهولة الحال حتى الاثنين —
        # ولو كانت معطّلة (صلاحيات، حماية فرع) لاكتشفناها بعد ضياع أول يوم من
        # أسبوع القياس كله.
        if not SIGNALS_CSV.exists():
            from tracker import _write_all
            _write_all([])          # ملف بترويسة فقط
        if state_store.enabled():
            if state_store.push(SIGNALS_CSV, message="فحص مزامنة سجل الإشارات"):
                logger.info("✅ مزامنة السجل تعمل — التقرير سيغطي اليوم كاملاً")
            else:
                logger.error(
                    "⚠️ مزامنة السجل لا تعمل — التقرير سيغطي التشغيل الحالي فقط"
                )
