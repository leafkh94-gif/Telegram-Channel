...
def build_daily_report() -> str:
    ...
    # نقطة التعادل تُشتق من الإعدادات لا تُكتب يدوياً: تغيير الوقف أو الهدف
    # يغيّرها، وسطر ثابت كان سيعطي حكماً خاطئاً بصمت بعد أي تعديل.
    if s["wins"] + s["losses"]:
        from config import ATR_SL_MULT, ATR_TP_MULT
        rr = ATR_TP_MULT / ATR_SL_MULT
        be = 100 / (1 + rr)
        verdict = "فوق التعادل ✅" if s["win_rate"] > be else "تحت التعادل ⚠️"
        lines += [
            f"محصلة R     : <b>{s['total_r']:+.2f}R</b>",
            f"التعادل عند {be:.0f}% ⇒ <b>{verdict}</b>",
        ]
